...
def Source_Region_Generator_Radius_Modifed_V3(Gname,Outpath,M=10):
    ...
    """
    '''
    # This old version only excludes background sources that are traced NOT all wavdetect sources, This needs to be fixed to include all sources ! ! !
    ...
    for Reg_File_L in Reg_File_H_L:
        ...
        Cur_Reg_File=open(Cur_Reg_Filepath)
        Cur_Reg_Str=Cur_Reg_File.read()
        #print "Cur_Reg_Str: \n", Cur_Reg_Str
        Coord_Str_L=re.findall('\([^\)]*\)',Cur_Reg_Str)
        #print "Coord_Str_L: \n", Coord_Str_L
        Shape_String_List=[]
        for Coord_Str in Coord_Str_L:
            ...
            Cur_Maj_Ax_Str=Coord_Str_Redueced_L[2]
            Cur_X=float(Cur_X_Str)
            Cur_Y=float(Cur_Y_Str)
            Cur_Maj_Ax=float(Cur_Maj_Ax_Str)
            Cur_Shape='circle('+str(Cur_X)+','+str(Cur_Y)+','+str(Cur_Maj_Ax * M) + ')' + '\n' #Cur_Shape:-str, Current_Shape, The big region (called a shape) of the current X-ray object, 'Big' refers to the multiplied radius #Note: Multiplier needs to be checked due to switching from Raytaced regions to Wavdect regions, It may have to be a different value other than 10
            Shape_String_List.append(Cur_Shape)
        #Region_fname=Gname_Underscore+"_"+"ObsID_"+Cur_Obs_ID_Str+"_Source_Regions_Radius_Modifed.txt"
        Region_fname=Gname_Modifed+"_ObsID_"+str(Cur_Reg_ObsID)+"_Source_Regions_Radius_Modifed.txt"
        ##Region_fpath=Source_Region_Tables_Filepath+Region_fname
        Region_fpath=Outpath+Region_fname
        #print "Region_fpath : ", Region_fpath
        file=open(Region_fpath,"w")
        #file.write("X"+","+"Y"+","+"Maj_Ax"+","+"Min_Ax"+","+"Angle")
        #file.write("X"+","+"Y"+","+"Maj_Ax")
        ...
        #This code needs to be tested, bug checked and finished ! ! ! !
...
#Source_Region_Generator_Radius_Modifed_V3("NGC 3631","Test_Fake_Path") #This is the current working version
