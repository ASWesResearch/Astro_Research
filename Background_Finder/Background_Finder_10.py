...
def Background_Finder_3(gname,evtfpath,objLfpath,R): #Need to apply energy filter (0.3kev to 10kev) to the counts, This may allow the code to treat back illuminated chips and front illuminated chips the same, if not then the code must be modifed to consider both cases
    ...
    if((Chip_ID_Aimpoint!=7) and (Chip_ID_Aimpoint!=5)):
        ...
        if(7 in Chip_ID_L):
            Chip_ID_Test_L.remove(7)
        Chip_ID_Test_L.remove(Chip_ID_Aimpoint)
        Chip_ID_Test_Reordered_Template_L=[0,1,2,3,6,8,4,9]
        Chip_ID_Test_Reordered_L=[Chip_ID_Aimpoint]
        for Chip_ID_Test_Reordered_Template in Chip_ID_Test_Reordered_Template_L:
    ...
    if(len(Background_L)<=3):
        for Step in Step_L:
            #print "Step ", Step
            #for Chip_ID_Test in Chip_ID_L: #Need to make sure the if the GC is on a front illuminated chip that only front illiminated chips are used (same for back illuminated chips, the all illumination chip types should match)
            for Chip_ID_Test in Chip_ID_Test_L:
                ...
                for c in range(0+BG_R,1025-BG_R,Step):   # c is "x"  #Check Bounds #The Bounds for CHIP coordinates are (1,1024)(both included), ie range(1,1025), So if this is correct (I am not 100% sure about these CHIP bounds), "for c in range(0+BG_R,1025-BG_R):" should instead be "for c in range(1+BG_R,1025-BG_R):"
                    for v in range(0+BG_R,1025-BG_R,Step): # v is "y"  #Check Bounds, should instead be "for v in range(1+BG_R,1025-BG_R):"(?)
                        ...
                        #print "Chip x is ",c
                        #print "Chip y is ",v
                        #dmcoords(infile=str(evtfpath),chipx=BG_X, chipy=BG_Y, chip_id=Chip_ID, option='chip', verbose=0) # Runs the dmcoords CIAO tool, which converts coordinates like CHIP_ID to SKY, The tool is now being used to convert the Background Circle center from CHIP to SKY coordinates (?)
                        dmcoords(infile=str(evtfpath),chipx=BG_X, chipy=BG_Y, chip_id=Chip_ID_Test, option='chip', verbose=0) # Runs the dmcoords CIAO tool, which converts coordinates like CHIP_ID to SKY, The tool is now being used to convert the Background Circle center from CHIP to SKY coordinates (?)
